...
export function startViewCollection(location, lifeCycle) {
    var startOrigin = 0;
...
function trackTimings(lifeCycle, callback) {
    ...
    var stopPerformanceTracking = lifeCycle.subscribe(LifeCycleEventType.PERFORMANCE_ENTRY_COLLECTED, function (entry) {
        if (entry.entryType === 'navigation') {
            timings = __assign(__assign({}, timings), { domComplete: entry.domComplete, domContentLoaded: entry.domContentLoadedEventEnd, domInteractive: entry.domInteractive, loadEventEnd: entry.loadEventEnd });
            callback(timings);
        }
        else if (entry.entryType === 'paint' && entry.name === 'first-contentful-paint') {
            timings = __assign(__assign({}, timings), { firstContentfulPaint: entry.startTime });
            callback(timings);
        }
    }).unsubscribe;
